...
resource "google_storage_bucket" "buckets" {
  ...
  name               = join("-", [var.prefix, lower(element(var.names, count.index))])
  ...
  versioning {
    enabled = lookup(var.versioning, lower(element(var.names, count.index)), false)
  }
}
...
resource "google_storage_bucket_iam_binding" "admins" {
  ...
  members = compact(concat(
    var.admins,
    split(",", lookup(var.bucket_admins, element(var.names, count.index), "")
    )
  ))
}
...
resource "google_storage_bucket_iam_binding" "creators" {
  ...
  members = compact(concat(
    var.creators,
    split(",", lookup(var.bucket_creators, element(var.names, count.index), "")
    )
  ))
}
...
resource "google_storage_bucket_iam_binding" "viewers" {
  ...
  members = compact(concat(
    var.viewers,
    split(",", lookup(var.bucket_viewers, element(var.names, count.index), "")
    )
  ))
}
